...
# Arquivo VLAEG v1 - progress

## 2026-07-10 - Plano local Platform Auth/Storage registrado

Registrado `project_state/plano_platform_admin_auth_storage.md` para adaptar o GennomX AI ao PLAN-008 da raiz. O plano preserva tokens MCP independentes, roles/RLS do PostgreSQL, trilha JWT proprio + MinIO ja em curso e autorizacao granular por scopes `ai:*`. Nenhum codigo, migracao, ingestao ou deploy executado nesta sessao.

---## Remocao total da Supabase — runtime JWT proprio + MinIO — 2026-07-09

Executado nesta sessao:

- `backend/app/config.py` passou a usar `AUTH_*` + `MINIO_*` e ganhou `urlsplit` na validacao de producao.
- `backend/app/auth/dependencies.py` ja opera com JWT HS256 local; os testes foram ajustados para o novo fluxo.
- `backend/workers/storage/raw_payload.py` e o deploy da VPS ficaram alinhados com MinIO como backend de storage.
- `frontend` perdeu os helpers Supabase legados, passou a usar cookie JWT proprio e teve `package-lock.json` atualizado.
- `docker-compose.yaml`, `infra/docker-compose.vps-app.yml` e `.env.example` foram migrados para `AUTH_*`/`MINIO_*`.
- `.github/workflows/ci.yml` e `frontend/playwright.config.ts` foram limpos de variaveis Supabase.
- Suite unitaria backend validada com `python -m pytest tests/unit -q`.
- Frontend validado com `npm run lint && npm run typecheck`.

Notas:

- Permanecem referencias historicas a Supabase em docs/arquivos de auditoria e em testes/guards legados de URL de banco, mas nao ha mais dependencia ativa de Supabase Auth/Storage no runtime.

---

## Plano aprovado - correcao dos issues de ingestao com dry-run - 2026-07-10

Marcus aprovou a correcao dos issues de ativacao da ingestao com inclusao de **dry-run real**. Registrado o plano canonico em `project_state/plano_correcao_issues_ingestao_dry_run.md` e atualizada a fonte viva de tarefas em `project_state/task_plan.md`.

Decisoes de escopo:

- A trilha de substituicao de Supabase por JWT proprio + MinIO/S3-compatible ja esta sendo conduzida por outro agente; este plano nao deve remover Supabase, implementar JWT proprio nem implementar MinIO.
- A correcao deve preservar a trilha paralela e integrar-se por contrato: dry-run independente de storage e preflight de storage usando o provider ativo no momento.
- Prioridades aprovadas: ativacao auditavel dos seis `data_sources` implementados, `dry_run` nas seis tasks de ingestao, disparo admin controlado com `max_records`, preflight de storage/env, `WarehouseCoverageService`, agendamento posterior de pos-processamento e primeiro incremento CT.gov `resultsSection`.

Nao executado nesta sessao: nenhuma alteracao de codigo, nenhuma ingestao, nenhum dry-run real, nenhum deploy e nenhuma mudanca em auth/storage. Esta rodada foi documental/planejamento.
...
## 2026-07-10 - PLAN-008 P-0/P-1: verificador administrativo isolado e R2 definido

Inventariados JWT local, cookie legivel pelo frontend, tokens MCP, roles/RLS e storage atual. Adicionados `app/auth/platform.py` e configuracoes `PLATFORM_AUTH_*` para verificar offline JWT RS256 de `gennomx-platform`; o modulo rejeita audience, tenant e scope incorretos. Ele nao foi conectado a `get_current_user`, frontend, dashboard, MCP ou RLS, portanto nao altera login local nem permissao de ingestao. Cloudflare R2 substitui MinIO como alvo de P-5, com buckets `gennomx-ai-{raw,processed,evidence}` e credenciais segregadas. O teste de Platform Auth passou (4/4); lint, typecheck e build do frontend tambem passaram. Sem bucket, migracao, ingestao, deploy ou cutover.
## 2026-07-10 - DRY-0 a DRY-4: freios operacionais da ingestao implementados

- Reconciliado o runtime: banco VPS, JWT proprio e MinIO/S3-compatible. A substituicao futura por
  Cloudflare R2 deve preservar o contrato S3-compatible; nenhuma dependencia Supabase foi
  reintroduzida.
- As seis tasks agora respeitam `data_sources.is_enabled` para execucao real. Fonte desativada vira
  `skipped` sem criar job nem escrever dados; dry-run administrativo permanece permitido para a
  allowlist dos seis conectores implementados.
- Implementado `dry_run` em CT.gov, PubMed, openFDA, DailyMed, Open Targets e EMA: fetch, parse e
  normalizacao executam; somente `IngestionJob` auditavel e gravado, com `dry_run_counts` em
  metadata. Nao ha escrita de raw, entidades, documentos, evidencias, assertions ou cursor.
- `JobTracker` agora preserva `partial` para limite/rejeicoes nao fatais, sem registrar falha nem
  avancar cursor. Dry-runs nao alteram a fonte.
- Implementado `WarehouseCoverageService` e `GET /api/v1/warehouse/coverage` (admin): inventario
  por entidade, rastreabilidade, fontes, jobs, conflitos e limitacoes explicitas. A API nao chama
  contagens internas de cobertura global de mercado.
- Adicionadas rotas admin auditaveis: ativacao por allowlist, trigger limitado com `max_records` e
  `dry_run`, e preflight de storage. O preflight MinIO escreve/le/remove objeto efemero antes de
  smoke-run real e e repetido como gate fail-closed pela task real; dry-run nao o chama.
- Atualizados `docs/03` e `docs/09`: Beat permanece desligado ate handshake, preflight, dry-runs e
  smoke-runs sequenciais revisados.
- Validacoes locais: ruff nos arquivos alterados; 9 testes novos/atualizados para controles de
  ingestao e 11 testes do retry de jobs, todos verdes com `STORAGE_BACKEND=minio`. O `.env` local
  ainda continha `STORAGE_BACKEND=supabase`; nao foi modificado por conter configuracao sensivel.

Proximo passo: executar handshake, preflight e dry-run dos seis conectores na VPS; ativar uma fonte
por vez somente apos revisao. DRY-6 e DRY-7 permanecem pendentes.
